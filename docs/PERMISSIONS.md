...
- `AgentCopilotPanel` answers `AGENT_EMAIL_NOT_VERIFIED` with the same offer even though it arrives over the WebSocket rather than as an HTTP error: it names the reason in the retry banner *and* calls `prompt()`. Only the transport differs, not what the user is offered. The agent store remembers which failure was already answered, so re-mounting the panel does not re-open the dialog for a refusal the user has already seen.
