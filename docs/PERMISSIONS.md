...
| `UpdateNoteHandler.execute`          | `PATCH /notes/:id` — an **owner** raising the link's exposure: opening it, or letting an open link write | Closing or lowering the link is exempt: an unverified owner must still be able to revoke a link they already exposed                            |
...
- At most 3 redirects, each back through the same check; http(s) only; no URL credentials; no cookie or auth header sent. One 10 s budget for the whole import, a client that disconnects stops it early, and the body stops at 10 MB.
