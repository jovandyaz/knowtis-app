...
3. Verify both public `kid` values with the [phase 3 public-only check](DEPLOYMENT.md#phase-3-confirm-overlap-publication). The MCP resource server verifies against `/oauth/jwks` through `jose.createRemoteJWKSet` with default options: an unknown `kid` causes a refetch only after the 30-second cooldown since the previous fetch has elapsed. During the cooldown verification fails; otherwise the next verification after the 10-minute cache maximum expires reloads the JWKS. Retry after the cooldown before switching signers; waiting 10 minutes or restarting the MCP service remains a conservative buffer.
4. In the Railway Dashboard, set API `OAUTH_JWKS` to exactly `{"keys":[NEW_JWK_OBJECT,OLD_JWK_OBJECT]}` (`[new, old]`) and deploy the API. Complete a normal MCP Inspector or client OAuth authorization-code + PKCE flow to mint a fresh token, then run the [phase 4 authenticated checks](DEPLOYMENT.md#phases-4-and-6-confirm-authenticated-access). If the protected-header `kid` is still the old one, stop: the authorization server did not pick the first key, which means a key's `alg` or `use` metadata differs, and the deployment should have failed validation.
