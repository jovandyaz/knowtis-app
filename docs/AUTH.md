...
| `RATE_LIMITING_ENABLED`  | No       | `true`                               | `false` skips every `@nestjs/throttler` route budget; the `/oauth` tiers and AI budgets are separate. For disposable test environments only, refused when `NODE_ENV=production`                                          |
