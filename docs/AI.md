...
`agent_history_injection_enforcement` is seeded **false**. The handler scans persisted assistant and tool messages before replay and emits `ai.input_guard.detected` metadata (surface, role, conversation ID, disposition, score, bounded content length and reason code). Content and raw flag-store errors are never included. Actual blocked rows additionally retain the `agent.history.message_dropped` event with the same explicit metadata, including historic-user drops while enforcement is off; observation emits only `ai.input_guard.detected`. With the flag off or unavailable, assistant/tool detections are observation only; unsafe historic user messages still drop. This default does not provide active replay protection.

When enabled, unsafe assistant/tool rows drop as a whole and both sides of orphaned tool pairs are repaired. Safe text survives repair without reviving a divergent hidden `content` value. Empty metadata-only assistant rows never reach the model. Projection follows assistant text parts and nested tool outputs, excluding tool-call inputs/IDs; it stops at 50,001 characters or 10,000 visited nodes. Oversized projections are rejected before heuristic scoring. The newest user request stays separate and retains its existing hard-fail guard and token accounting. Resume scans the persisted last user as history.

The `transcript-replay.eval.ts` promptfoo suite uses the pinned model, existing repeat/usage reporting, strict 100% graded pass rates for security cases and the standard two-thirds threshold for behavior. A deterministic real-SDK fixture exercises the same harness without paid provider calls. Current heuristic calibration rejects legitimate quotations of injection phrases (including the `amber lantern` glossary fixture): this is recorded as lost useful context, not reclassified as a security success. Keep enforcement disabled and issue #384 open until representative live-model calibration and an explicitly authorized rollout pass. Rollback is disabling the flag; persisted transcript rows remain unchanged.
