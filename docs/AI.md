...
| `AGENT_EDIT_WOULD_LOSE_CONTENT`   | `proposeEditNote` was asked to edit a note the converter pair cannot rebuild without dropping a node or mark (today: see the list above); the edit is refused rather than applied, and the message names the node types and tells the model to say the note must be edited by hand. `proposeUpdateNote` refuses with the same code a rewrite that would drop an AI block the user has not inserted or discarded; that message tells the model to ask the user to insert or discard it first |
