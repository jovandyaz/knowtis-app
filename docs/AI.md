...
- Aborts (user cancel, the `AI_AGENT_MAX_MS` ceiling) never advance the chain. A per-candidate **stall** does: it aborts only that candidate's signal, so the chain fails over to the next model (see the copilot agent's stall detection).
...
**Stall detection, not a wall-clock budget.** A reasoning model can legitimately work for minutes, so the orchestrator does not cap a turn by elapsed time — it caps **silence**. Each candidate gets its own `AbortController` and a `AI_AGENT_STALL_MS` timer that every `fullStream` part re-arms (reasoning, text, tool and step events all count as activity). Only a candidate that emits nothing for the whole budget is aborted; `AI_AGENT_MAX_MS` remains as an absolute backstop against a runaway turn that never stops producing parts.
