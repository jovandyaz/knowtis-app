...
3. `ai_default_model` (DB `ai_config` row, else the code default) — the system default.
...
**A shorter budget for the first part.** The first silence window of an attempt — before any part has arrived — is bounded by `AI_AGENT_TTFT_MS` (default 30 s), not the full `AI_AGENT_STALL_MS`: a candidate that hasn't said anything yet is far more likely to be dead than one already generating. The first non-marker part received flips the watchdog over to the `AI_AGENT_STALL_MS` budget for the rest of the attempt; a stall after that point follows the pre-existing semantics above, unchanged.

**Zero-output retry.** An attempt that hits the TTFT deadline without streaming a single part is retried against the **same candidate** exactly once — a fresh `streamText` call with identical inputs — because a request that streamed zero parts also ran zero tools and rendered zero text: rerunning it is idempotent, unlike retrying after any output has already streamed. The retry is logged as `agent.turn.retry` (`model`, `attempt`, `reason: 'ttft'`). If the retry is also silent, it falls through to the fresh-failure rules above unchanged: `AgentStallError` (chain advances) for a non-last, non-BYOK candidate; `AI_TIMEOUT` for the last candidate or a BYOK turn. A user abort or the `AI_AGENT_MAX_MS` ceiling firing during either attempt always wins over the retry, ending the turn immediately regardless of how many parts have streamed.
