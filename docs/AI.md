...
| `email_verification_gate`    | Product flag; when on, storing a BYOK key (`ByokService`) and approving a copilot `share` proposal (`ApproveMutationHandler`) require a verified, non-anonymous account (`VerifiedIdentityPolicy`) |
...
| `AGENT_EMAIL_NOT_VERIFIED`    | `share` approval by an unverified account while `email_verification_gate` is on (`AGENT_EMAIL_NOT_VERIFIED_CODE`, `@knowtis/shared-types`) |
