...
| `AI_AGENT_BYOK_MAX_STEPS`       | `20`     | Max tool-call loop iterations per BYOK turn, which has no `AI_AGENT_TURN_TOKEN_BUDGET`. Server-billed web calls still accrue on every step (see [Billing & rate limiting](#billing--rate-limiting)).                                                 |
...
| `AI_AGENT_TURN_TOKEN_BUDGET`    | `150000` | Ceiling on accumulated `inputTokens` + `outputTokens` per turn; once spent, the loop stops advancing to the next step. Anonymous turns are clamped to `min(budget, AI_DAILY_TOKEN_LIMIT × AI_ANONYMOUS_DAILY_LIMIT_PCT)`; BYOK turns have no budget. |
...
Env: `AI_AGENT_MAX_STEPS`, `AI_AGENT_BYOK_MAX_STEPS`, `AI_AGENT_TTFT_MS`, `AI_AGENT_STALL_MS`, `AI_AGENT_MAX_MS`, `AI_AGENT_MAX_OUTPUT_TOKENS`, `AI_AGENT_TURN_TOKEN_BUDGET`, `AI_AGENT_HISTORY_LIMIT`, `AI_AGENT_PROPOSAL_TTL_SECONDS` — see [Environment Variables](#environment-variables).
