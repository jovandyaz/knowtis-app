...
> **Classification requests scope fallback to the primary's family.** `resolveChainCandidates` accepts a `scope`: `'same-provider'` drops cross-provider chain entries before the availability and cooldown passes (the primary always survives, so a request still gets one attempt). `suggest-organization` asks for it via `StructuredOutputOptions.fallbackScope` — its bucket is persisted as the user's data, and a cross-family fallback swaps the classifier mid-flight, so the same note would land in a different bucket depending on which provider was healthy. When the primary family is down, the pass degrades to a failed suggestion instead of an inconsistent one. Transient completion actions (ghost text) deliberately keep the full chain for now (#327).
