...
**A `proposeEditNote` proposal is still a whole-body replacement, not a surgical patch.** The builder converts the stored HTML to Markdown, applies the edits, and converts the result back to HTML, so approving it rewrites the note's entire content — and whatever that converter pair cannot carry is lost even in the untouched parts. Today that means **images are dropped**, **nested task lists come back flat** (a sub-item becomes a sibling), and a **mermaid fence gains a blank line** before its closing delimiter. Tables, links, marks, and top-level task lists survive. The conversion is `packages/note-markdown` plus the `html-sanitizer` allowlist; anything added to one must be added to the other, or the edit path quietly deletes it.
