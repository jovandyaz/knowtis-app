...
| `apps/backoffice/vercel.json`          | Backoffice: build, SPA rewrite, security headers                                                                                                                                                                                                           |
...
- `Content-Security-Policy`: enforced, with two directives. `img-src` holds the same sources as the report-only `img-src`. `upgrade-insecure-requests` lives here because browsers ignore it in a report-only policy. The header carries no report directive: an image its `img-src` blocks also violates the report-only policy's identical `img-src`, which reports it.
- `Content-Security-Policy-Report-Only`: the full policy. `img-src` allows only the app, `data:`, `blob:` and the blob store host (`STORED_IMAGE_HOST`); `connect-src` allows only the app and `api.knowtis.app` (`https` and `wss`).
...
`apps/backoffice/vercel.json` sends the same set, pinned by `apps/backoffice/src/security-headers.test.ts`, with four differences: `connect-src` is the app and `https://api.knowtis.app` only (the backoffice opens no websocket), `img-src` drops `blob:`, `Permissions-Policy` denies the microphone too, and framing is refused outright (`X-Frame-Options: DENY`, `frame-ancestors 'none'`). Its `index.html` must carry no `<base>`, which `base-uri 'none'` would report on every load. Its reports reach the API through the backoffice origin, so production `BACKOFFICE_URL` must match `https://backoffice.knowtis.app` exactly, for the same reason as `FRONTEND_URL` below. Both apps make zod jitless in `src/lib/zod-config.ts`, imported by `main.tsx` before any app or workspace module (`src/zod-config-order.test.ts`); otherwise zod probes `new Function` on its first object parse, which `script-src 'self'` reports and, once enforced, blocks.

The CSP rolls out in two steps:

1. **Report-only, with `img-src` and `upgrade-insecure-requests` enforced (current).** Browsers block images from outside `img-src`, which stops the mermaid statements that fetch an image while drawing ([AI.md](AI.md)); they block nothing else, and report every violation to the API's `POST /api/v1/csp-reports` (no auth, 64 KB body limit, 30 requests per minute per client IP). The API logs each distinct violation once per request as a WARN `security.csp.violation` with `effectiveDirective`, `blockedSource`, `documentPath`, `disposition` and `count`. `blockedSource` keeps only the scheme, the last two host labels (deeper ones become `*.`) and the port, because the path, the query and the subdomain labels of a blocked URL can all carry exfiltrated data; `documentPath` replaces ids and share tokens with `:param`. Past 20 distinct violations in one request, a single `security.csp.violations_truncated` line counts the rest. Nothing is stored; filter them in Railway's log explorer with `@event:security.csp.violation`.
2. **Enforce the rest.** After 7 days of production reports with no unexpected `security.csp.violation`, move `upgrade-insecure-requests` into the full policy, delete the enforced two-directive header and rename `Content-Security-Policy-Report-Only` to `Content-Security-Policy` in `vercel.json` and in the header test, and the same in `apps/backoffice/vercel.json` and its test once its own 7 days are clean. Keep `report-uri`/`report-to` so violations keep arriving. From then on the PostHog Toolbar no longer works: it would need `https://us.posthog.com` in `connect-src`, `font-src` and `style-src`, and its images already fail because `img-src` stays pinned to the blob store.
...
A browser revalidating `index.html` gets a `304` from Vercel that carries none of these headers, and keeps the ones stored with its cached copy. New headers therefore reach a returning visitor only when `index.html` itself changes. The build stamps `GITHUB_SHA` into the bundle (`VITE_APP_VERSION`), so every frontend deploy changes the bundle's file name and with it `index.html`; a local build without `GITHUB_SHA` does not. The backoffice, whose bundle does not read the version, stamps it into `index.html` itself as `<meta name="release">` (`apps/backoffice/vite.config.ts`). Each deploy job fails if its build lacks the stamp.

#### Pasted images

`img-src` blocks an image hot-linked from another site, so the editor copies what a user pastes into the blob store instead. After a paste, a drop or a Markdown paste, the pasting client sends each foreign `http(s)` image to `POST /api/v1/notes/:id/images/import` ([SSRF controls](PERMISSIONS.md#importing-an-image-from-a-url)) and each pasted `data:` image to the upload endpoint, then swaps the image's `src` for the `STORED_IMAGE_HOST` URL. An image the API cannot copy (unreachable, over 10 MB, not PNG, JPEG, GIF or WebP) stays in the note as a link to its original URL, and the user gets one toast per paste. An editor who reaches the note only through its share link cannot store images (both endpoints answer 403), so their editor copies nothing: a foreign image they paste stays as the "Image unavailable" placeholder, a pasted `data:` image is dropped, and no toast appears. Both endpoints need `VERCEL_BLOB_READ_WRITE_TOKEN`. After a deploy that touches this path, paste an image from Wikipedia into a note and confirm its `src` moves to `STORED_IMAGE_HOST`.
...
[`.railway/railway.ts`](../.railway/railway.ts) is Railway Infrastructure as Code for the whole production environment: both services (`knowtis_app`, `knowtis-mcp`), their Dockerfile builds (see [Images](#images)), start commands, `/api/v1/health/ping` and `/health` healthchecks, restart on failure with 3 retries (`restartPolicyMaxRetries`; the `ON_FAILURE` type is Railway's default and is left implicit because `railway config plan` normalizes defaults to null), a 10 s draining time between Railway's `SIGTERM` and `SIGKILL` (`drainingSeconds`; the default is 0 s, which would kill the API before its shutdown hooks flush pending document stores, product analytics, and traces, and the MCP before it drains in-flight requests), and the API's pre-deploy migration command. The non-secret runtime variables (`NODE_ENV`, `PORT`, and `MCP_ALLOWED_HOSTS` on the MCP) are declared in the file; every other variable, secrets included, is `preserve()`: its value lives only in Railway and the file merely references it. Read the file rather than a copy here.

The file is applied by `railway config apply`, not read at deploy time, and Railway redeploys a service whose settings the apply changed. That redeploy rebuilds the service's most recent deployment snapshot — the code of the last `railway up`, not the merge commit — so a setting that needs a file the same change introduces (a new `dockerfilePath`, a new pre-deploy script) must merge after a deploy that already shipped that file. The reverse holds for rollbacks: a dashboard rollback restores a deployment's image and variables but not service settings, so rolling back across a builder change leaves the new builder and pre-deploy command in place. Undo such a change by reverting the `.railway/railway.ts` commit instead, and let the apply restore the old settings. `.github/workflows/railway-config.yml` plans every PR that touches `.railway/**` (the plan is posted as a PR comment together with the pinned tree, etag and change-set hash) and applies exactly that plan on merge; if the environment or `.railway/` moved in between, the apply fails and the PR needs a fresh plan. Destructive changes (a resource or variable removed from the file) are refused by the workflow; an intended deletion is applied locally after review with `railway config apply --confirm-destructive`. Locally: `railway config plan` previews, `railway config apply` applies. Config changes therefore flow through this workflow while code changes flow through `railway-deploy.sh`; the file is deliberately not an Nx input of any project. `railway.toml` config-as-code was retired in favour of this file (Railway stops reading it on 2026-12-01).
...
Both services build from multi-stage Dockerfiles, [`apps/api/Dockerfile`](../apps/api/Dockerfile) and [`apps/mcp/Dockerfile`](../apps/mcp/Dockerfile), on `node:24-slim` pinned by digest (Renovate refreshes the digest; the Node major moves by hand like everywhere else). The build stage installs the whole workspace with the committed lockfile and runs the same Nx build targets as CI, except that the API build runs with `NODE_ENV=production` (CI leaves it unset); the runtime stage keeps only what the process loads, running as the image's non-root `node` user.
...
| `IMAGE_IMPORT_ALLOWED_IPS`     | Never set in production      | Comma-separated IPs/CIDRs an image import from a URL may reach although they are private or loopback, so a local or E2E harness can import from `127.0.0.1`. The API refuses to boot with it set when `NODE_ENV=production` |
...
| API deploy fails health check right after editing `OAUTH_JWKS` | The environment validation error names `OAUTH_JWKS`; the value is set but a member is not EC/P-256/ES256/sig, lacks valid private signing material or a unique `kid`, or is not valid JSON. Fix the variable and redeploy; the previous deployment stays live. The public `jq` check below cannot catch this before deploy because the endpoint reflects only the currently running deployment's keys. |
...
After this check passes, either wait the full 10-minute remote-JWKS cache maximum age or restart the MCP service before switching the signer to `[new, old]`. A restart begins with an empty in-memory JWKS cache. Without a restart, the first token signed by the new key triggers the stale-cache refresh after the full maximum age has elapsed.
