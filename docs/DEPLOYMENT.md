...
2. **Enforce the rest.** After 7 days of production reports with no unexpected `security.csp.violation`, move `upgrade-insecure-requests` into the full policy, delete the enforced two-directive header and rename `Content-Security-Policy-Report-Only` to `Content-Security-Policy` in `vercel.json` and in the header test. Keep `report-uri`/`report-to` so violations keep arriving. From then on the PostHog Toolbar no longer works: it would need `https://us.posthog.com` in `connect-src`, `font-src` and `style-src`, and its images already fail because `img-src` stays pinned to the blob store.
