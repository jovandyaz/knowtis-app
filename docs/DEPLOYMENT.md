...
- `Content-Security-Policy`: enforced, with two directives. `img-src` holds the same sources as the report-only `img-src`. `upgrade-insecure-requests` lives here because browsers ignore it in a report-only policy. It carries no report directive: an image it blocks also violates the report-only policy's identical `img-src`, which reports it.
