...
- `Content-Security-Policy`: enforced, with `img-src` as its only directive, holding the same sources as the report-only `img-src`. It carries no report directive: an image it blocks also violates the report-only policy's identical `img-src`, which reports it.
