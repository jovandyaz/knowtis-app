import { TOUCH_TARGET_HEIGHT_CLASS } from '@knowtis/design-system';

export const NAV_ICON_SLOT = 'flex w-4 shrink-0 items-center justify-center';

export const NAV_ROW = `flex min-h-8 items-center gap-2 rounded-md px-2 py-1 text-sm transition-colors cursor-pointer ${TOUCH_TARGET_HEIGHT_CLASS}`;

export const NAV_ROW_ACTIVE = 'bg-muted text-foreground font-medium';

export const NAV_ROW_IDLE =
  'text-foreground dark:text-muted-foreground hover:bg-primary/5 hover:text-primary';

export const NAV_LABEL = 'min-w-0 flex-1 truncate';

export const NAV_COUNT =
  'shrink-0 text-xs tabular-nums text-foreground dark:text-muted-foreground';
