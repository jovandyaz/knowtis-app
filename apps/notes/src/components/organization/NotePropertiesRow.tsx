...
const ROW_CLASSES = 'mt-1 mb-6 flex flex-wrap items-center gap-x-3 gap-y-2';
...
export function NotePropertiesRow({
  ...
  return (
    <div className={ROW_CLASSES}>
      <div className={GROUP_CLASSES}>
        <DropdownMenu>
          <DropdownMenuTrigger className={MENU_CHIP_CLASSES}>
            <BucketDot bucket={activeFilter} />
            {label}
      ...
      <div className={GROUP_CLASSES}>
        {tags.map((path) => (
          <button
            ...
            className={`${MENU_CHIP_CLASSES} cursor-pointer`}
          >
            <Hash className="size-3 opacity-60" />
            {path}
        ...
        <TagPicker
          ...
          triggerClassName={ADD_CHIP_CLASSES}
        />
      </div>
