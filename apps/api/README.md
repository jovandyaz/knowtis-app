...
├── core/           # Cross-cutting: auth, domain, errors, exceptions, filters,
│                   # http, interceptors, pagination, swagger, throttling, validation
...
- Rate limit: 60 requests per 60 s window, keyed per registered user; anonymous callers are keyed per client IP — `core/throttling/`. The IP is `clientIpOf()` (`core/http/client-ip.ts`): Railway's edge-set `X-Real-IP`, which the edge overwrites on every request, never `X-Forwarded-For`; `req.ip` is only the fallback where no edge stamps the header (local development, private-network callers). The `/oauth` express-rate-limit tiers use the same key. Counters are in-memory per replica and reset on every rollout. `RATE_LIMITING_ENABLED=false` skips every `@nestjs/throttler` route budget for disposable test environments and is refused in production; the `/oauth` tiers and AI budgets are separate.
...
Production runs on Railway from the multi-stage [`Dockerfile`](Dockerfile) (builder declared in `.railway/railway.ts`). Build with `pnpm build:api` (output `dist/apps/api/main.js`), start with `node dist/apps/api/main.js`; migrations run in Railway's `preDeployCommand` through `apps/api/src/database/migrate.cjs`, the image's bundled copy of `migrate.ts`. See [docs/DEPLOYMENT.md](../../docs/DEPLOYMENT.md#images).
