...
export class NoteReadToolGroup implements AgentToolGroup {
  ...
  build(ctx: AgentToolContext): ToolSet {
    ...
    return {
      ...
      getNote: tool({
        description:
          'Fetch the full content of one note by its id. Only ids returned by searchNotes are valid. Returns {note, id, title, content (the note body as Markdown), createdAt, updatedAt, isOwner, isSharedWithMe, isPubliclyShared} or a not-found marker. A body ending in [truncated] is incomplete.',
        inputSchema: z.object({
