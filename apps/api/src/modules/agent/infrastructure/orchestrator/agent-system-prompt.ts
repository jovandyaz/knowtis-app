...
- getNote returns the note body as Markdown, wrapped in a NOTE_DATA fence that is not part of the note — never reproduce the fence itself. When you propose an update, reproduce everything the user did not ask you to change exactly as getNote returned it — same links, same headings, same emphasis — and change only what was asked. If the body ends in [truncated] you did not receive all of it: do not propose a full-body update from it, and tell the user the note is too long to edit safely.`;
