...
`agent_history_injection_enforcement` is seeded **false**. The handler scans persisted assistant and tool messages before replay and emits one aggregated `ai.input_guard.detected` event per turn (surface, user ID, conversation ID, observed and blocked counts, plus a per-row role, disposition, score, bounded content length and reason code); a turn that blocked at least one row adds a single `agent.history.message_dropped` count event, and a dropped coalesced user turn adds a single `agent.history.user_turn_dropped` event with its score and content length. The aggregation is deliberate: persisted rows are rescanned on every replay, so a per-row emission turns one poisoned row into an unbounded stream of identical warnings. Content and raw flag-store errors are never included. With the flag off or unavailable, assistant/tool detections are observation only; unsafe historic user messages still drop. This default does not provide active replay protection.

When enabled, unsafe assistant/tool rows drop as a whole and both sides of orphaned tool pairs are repaired. Safe text survives repair without reviving a divergent hidden `content` value. Empty metadata-only assistant rows never reach the model. Projection follows exactly what `toModelMessages` replays — assistant text parts, tool-call inputs and nested tool outputs — excluding tool call IDs and tool names; it stops one character past the guard scan limit, or at 10,000 visited nodes. `detectAiInput` normalizes that projection and collapses its whitespace runs once, then scans the collapsed copy in windows of `MAX_GUARD_INPUT_CHARS` overlapping by `MAX_INJECTION_PATTERN_SPAN_CHARS`, unioning the patterns each window matched — so a long legitimate note is scanned instead of refused for its size, and weak signals still accumulate across windows. Collapsing first is what makes the overlap a real bound for the windowed patterns: their bridging quantifiers are capped, `MAX_INJECTION_PATTERN_SPAN_CHARS` is computed from those caps, and padding — whitespace, or the invisible characters `normalizeForGuard` strips — cannot push a phrase across a seam. The one pattern whose match must cover a whole character run (`runAnchored`, the weak base64 signal) is exempt from that bound and is scanned once over the entire collapsed text instead, because a window cut would otherwise satisfy its lookarounds vacuously and manufacture a hit on a legitimate row. The windowed score is therefore at least, not always equal to, what a single unbounded scan would give: collapsing a newline to a space can also let `\bDAN\b.{0,200}mode` bridge two lines. Only a projection past `MAX_GUARD_SCAN_CHARS`, measured on the raw text and again after normalization so an NFKC expansion cannot add windows — which the node-exhaustion sentinel deliberately exceeds — is refused unscanned as `too_large`. The newest user request keeps its own hard-fail guard and token accounting, measured on its own length rather than on the merged history. Because the provider is handed consecutive user rows merged with a blank line, the final user turn is guarded a second time as that joined text; a bad verdict drops the persisted half (`agent.history.user_turn_dropped`) instead of failing the turn. Only the seam is re-scanned — `COALESCED_SEAM_CHARS` from each side, trimmed to a whitespace boundary so the cut cannot manufacture a partial-run hit — because both halves are already guarded on their own and the whole join would exceed `InjectionGuardService`'s single-window input limit; the logged `contentLength` is still the real joined length. Resume applies the same full guard, classifier included, to the last persisted user row.
