...
Framework-free core: messages (`agent-message`, `coalesce-messages` for provider alternation), `proposed-mutation`, `retrieval`, `memory-reconcile`, `agent-event`, `agent-errors`. Ports under [`domain/ports/`](domain/ports):
...
| Subfolder       | Contents                                                                                                                                                                                                     |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orchestrator/` | `ai-sdk-agent.orchestrator` (Vercel AI SDK tool-use loop), `agent-tool.registry`, `agent-system-prompt` + `compose-system-prompt`, `proposal-collector`, `mutation-proposal.builder`, `web-source.collector` |
...
Eval suites live in [`eval/`](eval): `copilot.eval`, `memory-recall.eval`, `retrieval-quality.eval`, `web-search-quality.eval` (with `eval/fixtures/` note sets and an `eval/runtime/` bootstrap).
