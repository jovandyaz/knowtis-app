...
When enabled, unsafe assistant/tool rows drop as a whole and both sides of orphaned tool pairs are repaired. Safe text survives repair without reviving a divergent hidden `content` value. Empty metadata-only assistant rows never reach the model. Projection follows exactly what `toModelMessages` replays — assistant text parts, tool-call inputs and nested tool outputs — excluding tool call IDs and tool names; it stops one character past the guard scan limit, or at 10,000 visited nodes. `detectAiInput` normalizes that projection and collapses its whitespace runs once, then scans the collapsed copy in windows of `MAX_GUARD_INPUT_CHARS` overlapping by `MAX_INJECTION_PATTERN_SPAN_CHARS`, unioning the patterns each window matched — so a long legitimate note is scanned instead of refused for its size, and the cumulative score is the one a single unbounded scan would produce. Collapsing first is what makes the overlap a real bound: every bridging quantifier in `INJECTION_PATTERNS` is capped, so no match spans more than `MAX_INJECTION_PATTERN_SPAN_CHARS` of collapsed text, and padding — whitespace, or the invisible characters `normalizeForGuard` strips — cannot push a phrase across a seam. Window cuts carry a sentinel character so the base64 signal's lookarounds cannot succeed vacuously on a run the cut split. Only a projection past `MAX_GUARD_SCAN_CHARS` — which the node-exhaustion sentinel deliberately is — is refused unscanned as `too_large`. The newest user request keeps its own hard-fail guard and token accounting, measured on its own length rather than on the merged history. Because the provider is handed consecutive user rows merged with a blank line, the final user turn is guarded a second time as that joined text; a bad verdict drops the persisted half (`agent.history.user_turn_dropped`) instead of failing the turn. Only the seam is re-scanned — `COALESCED_SEAM_CHARS` from each side — because both halves are already guarded on their own and the whole join would exceed `InjectionGuardService`'s single-window input limit. Resume applies the same full guard, classifier included, to the last persisted user row.
