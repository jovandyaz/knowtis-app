...
The promptfoo suites (`copilot.eval`, `injection-guard.eval`) are multi-trial: `AI_EVAL_TRIALS` (default 1, nightly CI 3) repeats every case and a case fails below a `ceil(2/3 × trials)` per-case pass threshold. At the nightly's three trials that tolerates one flaky trial without excusing a consistent regression; at one or two trials every trial must pass.
